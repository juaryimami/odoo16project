...
class ConstructionDashboard(models.Model):
    ...
    def get_construction_statistics(self):
        ...
        # Cost sheets derived from Job Orders of construction projects
        job_orders = self.env['construction.job.order'].search([('project_id', 'in', project_ids)])
        cost_sheet_ids = job_orders.mapped('cost_sheet_id').ids
        estimate_count = len(set(cost_sheet_ids))
        ...
        total_collected_revenue = sum(proj.total_collected_revenue for proj in construction_projects)
        ...
        # 1.2 Multi-Project Sold Unit COUNT Analysis (Last 10 Months)
        unit_count_labels = []
        unit_count_datasets = []
        for i in range(9, -1, -1):
            label_date = today - relativedelta(months=i, day=1)
            unit_count_labels.append(label_date.strftime('%b %y'))
        ...
        for proj in construction_projects:
            ...
            for i in range(9, -1, -1):
                ...
                monthly_count = self.env['sale.order'].search_count([
                    ('state', 'in', ['sale', 'done']),
                    ('project_id', '=', proj.id),
                    ('unit_id', '!=', False),
                    ('date_order', '>=', start_date),
                ...
                project_unit_counts.append(monthly_count)
            
            if sum(project_unit_counts) > 0:
                unit_count_datasets.append({
                    'label': proj.name,
                    'data': project_unit_counts
                })
        ...
        for line in mat_lines:
             ...
             if po_lines == 0:
                 risks.append({
                     ...
                     'severity': 'high' if (line.procurement_lead_time or 0) > 10 else 'medium'
                 })
        risks = risks[:5]
        ...
        # 7. Category Burn Data (Budget vs Actual) - Pulled from Master Project Plan
        summary_lines = self.env['project.master.plan.summary'].search([('project_id', 'in', project_ids)])
        
        def get_summary_total(label, field):
            return sum(summary_lines.filtered(lambda l: l.item_type == label).mapped(field))

        burn_data = {
            'labels': ['Material', 'Labor', 'Equipment', 'Fleet', 'Overhead'],
            'budget': [
                get_summary_total('Material', 'total_budget'),
                get_summary_total('Labor', 'total_budget'),
                get_summary_total('Equipment', 'total_budget'),
                get_summary_total('Fleet', 'total_budget'),
                get_summary_total('Overhead & Admin', 'total_budget'),
            ],
            'actual': [
                get_summary_total('Material', 'actual_spent'),
                get_summary_total('Labor', 'actual_spent'),
                get_summary_total('Equipment', 'actual_spent'),
                get_summary_total('Fleet', 'actual_spent'),
                get_summary_total('Overhead & Admin', 'actual_spent'),
            ]
        }
        ...
        return {
            'company_currency': self.env.company.currency_id.name or 'AED',
            ...
            'charts': {
                ...
                'unit_count_analysis': {
                    'labels': unit_count_labels,
                    'datasets': unit_count_datasets
                },
