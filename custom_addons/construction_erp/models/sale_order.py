...
class SaleOrder(models.Model):
    ...
    installment_plan_count = fields.Integer(compute='_compute_installment_plan_count')
    installment_plan_ids = fields.One2many('real_estate.installment.plan', 'sale_id', string='Installment Plans')
    
    def _compute_installment_plan_count(self):
        for order in self:
            order.installment_plan_count = len(order.installment_plan_ids)

    has_portal_access = fields.Boolean(compute='_compute_has_portal_access', store=False)

    def _compute_has_portal_access(self):
        portal_group = self.env.ref('base.group_portal', raise_if_not_found=False)
        for order in self:
            if not portal_group:
                order.has_portal_access = False
                continue
            has_access = any(portal_group in user.groups_id for user in order.partner_id.user_ids)
            order.has_portal_access = has_access

    def action_generate_portal_access(self):
        self.ensure_one()
        return {
            'name': _('Generate Portal Access'),
            'type': 'ir.actions.act_window',
            'res_model': 'construction.portal.access.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
                'active_id': self.id,
    ...
    def action_confirm(self):
        for order in self:
            if order.unit_id:
                ...
                # 2. Check if installment plan already exists (Bypass if coming from wizard)
                if not self.env.context.get('skip_installment_wizard'):
                    plan = self.env['real_estate.installment.plan'].search([('sale_id', '=', order.id)], limit=1)
                    if not plan:
                        # Return the wizard to collect deposit and months
                        return {
                            'name': _('Configure Installment Plan'),
                            'type': 'ir.actions.act_window',
                            'res_model': 'real.estate.installment.wizard',
                            'view_mode': 'form',
                            'target': 'new',
                            'context': {
                                'default_sale_order_id': order.id,
                                'default_deposit_amount': order.amount_total * 0.1, # Suggested 10%
                            }
                        }
...
class SaleOrderLine(models.Model):
    ...
    move_ids = fields.Many2many('stock.move', compute='_compute_dummy_move_ids', string='Stock Moves')

    def _compute_dummy_move_ids(self):
        for line in self:
