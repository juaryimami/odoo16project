...
export class ConstructionDashboardMain extends Component {
    ...
    renderCharts() {
        ...
        if(burnCtx && window.Chart) {
            new Chart(burnCtx, {
                type: 'bar',
                data: {
                    labels: this.state.metrics.charts.burn_data.labels,
                    datasets: [
                        {
        ...
        // 3.1 Monthly Sold Unit Volume Pulse (Last 10 Months)
        const unitVolumeCtx = document.getElementById("monthlyUnitsSoldChart");
        if(unitVolumeCtx && window.Chart) {
            const rawDatasets = this.state.metrics.charts.unit_count_analysis.datasets || [];
            const palette = ['#1cc88a', '#4e73df', '#f6c23e', '#36b9cc', '#e74a3b', '#5a5c69', '#6610f2', '#e83e8c'];
            ...
            new Chart(unitVolumeCtx, {
                type: 'line',
                data: {
                    labels: this.state.metrics.charts.unit_count_analysis.labels,
                    datasets: datasets
                },
                options: {
                    responsive: true,
                    ...
                    scales: {
                        y: { 
                            beginAtZero: true, 
                            grid: { borderDash: [2, 2], color: '#f2f2f2' },
                            ticks: { precision: 0 } // Show integers only for unit counts
                        },
                        x: { grid: { display: false } }
                    },
                    plugins: {
    ...
    openEstimates() { this.action.doAction('construction_erp.action_construction_cost_sheet'); }
    ...
    openJobOrders() { this.action.doAction('construction_erp.action_construction_job_order'); }
    openMaterialRequests() { this.action.doAction('construction_erp.action_material_requisition'); }
    openAssets() { this.action.doAction('construction_erp.action_real_estate_units'); }
