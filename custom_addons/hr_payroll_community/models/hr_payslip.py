...
class HrPayslip(models.Model):
    ...
    def action_payslip_done(self):
        ...
        for payslip in self:
            if 'hr.overtime' in self.env:
                ...
                overtime_reqs = self.env['hr.overtime'].search(domain)
                if overtime_reqs:
                    overtime_reqs.write({'payslip_paid': True})
        return res
    ...
    def compute_sheet(self):
        if not self.env.context.get('skip_absence_wizard'):
            absent_payslips = self.filtered(lambda p: any(wd.code == 'ABSENT' and wd.number_of_hours > 0 for wd in p.worked_days_line_ids))
            if absent_payslips:
                html_message = """
                <div class="alert alert-warning" role="alert" style="margin-bottom: 20px;">
                    <h4 class="alert-heading" style="margin-bottom: 10px;"><i class="fa fa-exclamation-triangle"></i> Absences Detected!</h4>
                    <p>The following employees have recorded absences this month. Please review them below.</p>
                    <hr/>
                    <p class="mb-0"><strong>Do you want to deduct these absences from their salary or forgive them (issue warning)?</strong></p>
                </div>
                <table class="table table-sm table-bordered table-striped" style="width: 100%; border-collapse: collapse;">
                    <thead class="thead-light">
                        <tr style="background-color: #f8f9fa;">
                            <th style="padding: 8px; border: 1px solid #dee2e6;">Employee</th>
                            <th style="padding: 8px; border: 1px solid #dee2e6; text-align: center;">Absent Hours</th>
                            <th style="padding: 8px; border: 1px solid #dee2e6; text-align: center;">Absent Days</th>
                        </tr>
                    </thead>
                    <tbody>
                """
                for p in absent_payslips:
                    absent_line = p.worked_days_line_ids.filtered(lambda wd: wd.code == 'ABSENT')
                    if absent_line:
                        hours = sum(absent_line.mapped('number_of_hours'))
                        days = sum(absent_line.mapped('number_of_days'))
                        html_message += f"""
                        <tr>
                            <td style="padding: 8px; border: 1px solid #dee2e6;">{p.employee_id.name}</td>
                            <td style="padding: 8px; border: 1px solid #dee2e6; text-align: center;">{hours:.2f}</td>
                            <td style="padding: 8px; border: 1px solid #dee2e6; text-align: center;">{days:.2f}</td>
                        </tr>
                        """
                html_message += """
                    </tbody>
                ...
                wizard = self.env['absence.confirmation.wizard'].create({
                    'payslip_ids': [(6, 0, self.ids)],
                    'message': html_message
                })
                return {
                    'name': _('Absence Deduction Confirmation'),
                    'type': 'ir.actions.act_window',
                    'res_model': 'absence.confirmation.wizard',
                    'res_id': wizard.id,
                    'view_mode': 'form',
                    'target': 'new',
                }
    ...
    def get_worked_day_lines(self, contracts, date_from, date_to):
        ...
        for contract in contracts.filtered(
            ...
            # compute worked days from approved attendance
            start_dt = utc.localize(day_from)
            end_dt = utc.localize(day_to)
            work_intervals = calendar._work_intervals_batch(start_dt, end_dt, resources=contract.employee_id.resource_id)[contract.employee_id.resource_id.id]
            ...
            attendances_recs = self.env['hr.attendance'].search(domain)
            att_intervals_list = []
            for att in attendances_recs:
                if not att.check_out:
                    continue
                start_utc = max(day_from, att.check_in)
                end_utc = min(day_to, att.check_out)
                if start_utc >= end_utc:
                    continue
                att_intervals_list.append((utc.localize(start_utc), utc.localize(end_utc), att))
            
            from odoo.addons.resource.models.resource import Intervals
            attendance_intervals = Intervals(att_intervals_list)
            
            # Intersection of attendance and work schedule to get normal working hours
            normal_intervals = attendance_intervals & work_intervals
            
            normal_hours = sum((end - start).total_seconds() / 3600.0 for start, end, dummy in normal_intervals)
            # calculate days based on hours per day (defaulting to 8 if not defined)
            hours_per_day = calendar.hours_per_day or 8.0
            normal_days = normal_hours / hours_per_day if hours_per_day else 0.0
            ...
            # calculate absent days from absent attendance
            if 'state' in self.env['hr.attendance']._fields:
                absent_domain = [
                    ('employee_id', '=', contract.employee_id.id),
                    ('check_in', '<', day_to),
                ...
                absent_recs = self.env['hr.attendance'].search(absent_domain)
                absent_intervals_list = []
                for att in absent_recs:
                    if not att.check_out:
                        continue
                    start_utc = max(day_from, att.check_in)
                    end_utc = min(day_to, att.check_out)
                    if start_utc >= end_utc:
                        continue
                    absent_intervals_list.append((utc.localize(start_utc), utc.localize(end_utc), att))
                
                absent_attendance_intervals = Intervals(absent_intervals_list)
                absent_normal_intervals = absent_attendance_intervals & work_intervals
                absent_hours = sum((end - start).total_seconds() / 3600.0 for start, end, dummy in absent_normal_intervals)
                absent_days = absent_hours / hours_per_day if hours_per_day else 0.0
    ...
    def get_inputs(self, contracts, date_from, date_to):
        ...
        for contract in contracts:
            ...
            for input in inputs:
                ...
                contract_inputs.append(input_data)
            
            # Fetch approved cash overtime
            domain = [
                ('employee_id', '=', contract.employee_id.id),
                ('state', '=', 'approved'),
                ('type', '=', 'cash'),
            ...
            if 'hr.overtime' in self.env:
                ...
                if overtime_reqs:
                    ot_types = {
                        'OT_NORM': ('Normal Overtime', 'ot_normal_amount'),
                        'OT_NIGHT': ('Night Overtime', 'ot_night_amount'),
                        'OT_WEEK': ('Weekend Overtime', 'ot_weekend_amount'),
                        'OT_HOL': ('Public Holiday Overtime', 'ot_holiday_amount'),
                    }
                    for code, (desc, field) in ot_types.items():
                        total_amount = sum(overtime_reqs.mapped(field))
                        # Always remove any pre-populated zero-amount entries for this code to prevent duplication
                        contract_inputs = [i for i in contract_inputs if i.get('code') != code]
                        if total_amount > 0:
    ...
    def onchange_employee(self):
        ...
        worked_days_line_ids = self.get_worked_day_lines(contracts, date_from,
                                                         date_to)
        self.worked_days_line_ids = [(5, 0, 0)] + [(0, 0, r) for r in worked_days_line_ids]

        input_line_ids = self.get_inputs(contracts, date_from, date_to)
        self.input_line_ids = [(5, 0, 0)] + [(0, 0, r) for r in input_line_ids]
        return
