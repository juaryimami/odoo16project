...
class SalaryAdvancePayment(models.Model):
    ...
    currency_id = fields.Many2one('res.currency', string='Currency', related="employee_id.contract_id.currency_id")
    ...
    # employee_contract_id = fields.Char(string='Contract', related='employee_id.hr_contract.name')

    employee_contract_id = fields.Many2one('hr.contract', string="Contract", compute='_employee_contract_id', readonly=True,
                             store=True)

    @api.depends('employee_id')
    def _employee_contract_id(self):
        for contract in self:
            contract.employee_contract_id = self.env['hr.contract'].search([('employee_id','=',contract.employee_id.id),('state','=', 'open')])
    ...
    def submit_to_manager(self):
        self.state = 'submit'
        users = self.env.ref('ohrms_salary_advance.advance_group_manager').users
        for user in users:
            activity_type = self.env.ref('ohrms_salary_advance.mail_activity_advance_salary')
            self.activity_schedule('ohrms_salary_advance.mail_activity_advance_salary', user_id=user.id,
                                   note=f"Advance salary is submitted {self.employee_id.name}")

        # mail_template = self.env.ref('ohrms_salary_advance.advance_salary_email_template')
    ...
    def approve_request(self):
        ...
        for user in users:
            activity_type = self.env.ref('ohrms_salary_advance.mail_activity_advance_salary')
            self.activity_schedule('ohrms_salary_advance.mail_activity_advance_salary', user_id=user.id,
                                   note=f"Advance salary is checked {self.employee_id.name}")

        message = "Salary Advance is Successfully Checked"
        return {
    ...
    def approve_request_acc_dept(self):
        ...
        for request in self:
            ...
            draft.action_post()
            self.state = 'approve'
            users = self.env.ref('ohrms_salary_advance.advance_group_approver').users
            for user in users:
                activity_type = self.env.ref('ohrms_salary_advance.mail_activity_advance_salary')
                self.activity_schedule('ohrms_salary_advance.mail_activity_advance_salary', user_id=user.id,
                                       note=f"Advance salary is approved {self.employee_id.name}")
