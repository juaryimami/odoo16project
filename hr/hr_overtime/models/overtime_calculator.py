...
class OvertimeCalcualator(models.Model):
    ...
    request_id = fields.Many2one('overtime.request',string="Request",tracking=True)
    ...
    def _compute_value(self):
            for rec in self:
                if rec.employee_id.contract_id:
                    ...
                    type = self.env['overtime.rate'].search([('id','=',rec.overtime_type_id.id)])
                    if hour_count > 0:
                        salary_per_hour = contract.wage / (hour_count*4.29)
