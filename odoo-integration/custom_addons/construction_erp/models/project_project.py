...
class Project(models.Model):
    ...
    project_owner_id = fields.Many2one('res.users', string='Project Owner', tracking=True, help="Ultimate authority for final financial and operational approvals.")
    inspector_id = fields.Many2one('res.partner', string='Project Inspector', tracking=True, help="External or Internal party responsible for Quality Assurance & Sign-off.")
    ...
    total_committed_allocated = fields.Monetary(string='Total Committed (JO)', compute='_compute_project_financials')
    ...
    budget_utilization_pct = fields.Float(string='Budget Utilization', compute='_compute_project_financials')
    ...
    total_sold_count = fields.Integer(compute='_compute_sales_financials', string='Total Sold Units (Count)')
    ...
    gps_collector = fields.Boolean(string='GPS Collector', store=False)
    ...
    def action_view_tasks(self):
        """ Redirect native task view to construction job orders for construction projects. """
        self.ensure_one()
        if self.project_type == 'construction':
            return self.action_open_job_orders()
    ...
    master_plan_summary_ids = fields.One2many('project.master.plan.summary', 'project_id', string='Master Plan Summary', compute='_compute_plan_summary', store=True)
    ...
    @api.depends('job_order_ids', 
                 'job_order_ids.planned_material_cost', 'job_order_ids.actual_material_spend',
                 'job_order_ids.planned_labor_cost', 'job_order_ids.actual_labor_spend',
                 'job_order_ids.planned_fleet_cost', 'job_order_ids.actual_fleet_spend',
                 'job_order_ids.planned_equipment_cost', 'job_order_ids.actual_equipment_spend',
                 'job_order_ids.planned_overhead_cost', 'job_order_ids.actual_overhead_spend')
    def _compute_plan_summary(self):
        for project in self:
            job_orders = project.job_order_ids
            category_config = [
                ('Material', 'planned_material_cost', 'actual_material_spend'),
                ('Labor', 'planned_labor_cost', 'actual_labor_spend'),
                ('Equipment', 'planned_equipment_cost', 'actual_equipment_spend'),
                ('Fleet', 'planned_fleet_cost', 'actual_fleet_spend'),
                ('Overhead & Admin', 'planned_overhead_cost', 'actual_overhead_spend'),
            ]
            summary_vals = []
            for label, p_field, a_field in category_config:
                budget = sum(job_orders.mapped(p_field))
                actual = sum(job_orders.mapped(a_field))
                phase_names = set(job_orders.filtered(lambda jo: jo[p_field] > 0 or jo[a_field] > 0).mapped('phase_id.name'))
                phases_str = ", ".join(sorted(filter(None, list(phase_names))))
            ...
            project.master_plan_summary_ids = [(5, 0, 0)] + summary_vals

    def action_refresh_financials(self):
        """Force recalculation of all financial aggregates and summaries immediately."""
        self.ensure_one()
        # Trigger recomputation of stored fields by marking dependencies as modified
        self.modified(['job_order_ids'])
        self._compute_plan_summary()
        self._compute_aggregate_estimations()
        return True
    ...
    def _compute_sales_financials(self):
        for project in self:
            ...
            project.total_sold_count = len(confirmed_sales)
            
            # Total Revenue Collected = Sum of all posted invoices linked to this project (including partial payments)
            invoices = self.env['account.move'].search([
                ('project_id', '=', project.id),
                ('move_type', '=', 'out_invoice'),
                ('state', '=', 'posted')
            ])
            project.total_collected_revenue = sum(inv.amount_total - inv.amount_residual for inv in invoices)
    ...
    def _compute_project_financials(self):
        for project in self:
            ...
            # 2. Budget from Job Orders (as per user request for unified reporting)
            project.total_budget = sum(project.job_order_ids.mapped('total_planned_budget'))
            ...
            # 4. Committed & Actuals (Pulling from Job Orders for consistency with Master Plan)
            project.total_committed_allocated = sum(project.job_order_ids.mapped('total_planned_budget'))
            project.total_expense = sum(project.job_order_ids.mapped('total_actual_spend'))
            
            # 5. Revenue (From Analytic Lines / Invoices)
            if not project.analytic_account_id:
                project.total_collected = 0.0
                project.profitability = project.revised_budget_amount - project.total_expense
                project.budget_utilization_pct = (project.total_expense / project.revised_budget_amount * 100) if project.revised_budget_amount > 0 else 0.0
                continue
    ...
    def create(self, vals_list):
        ...
        for project in projects:
            ...
            if not project.analytic_account_id:
                # Find or create the construction analytic plan (Required for Odoo 16)
                plan = self.env.ref('construction_erp.analytic_plan_projects', raise_if_not_found=False)
                if not plan:
                    plan = self.env['account.analytic.plan'].search([('name', '=', 'Construction Projects')], limit=1)
            ...
            if project.project_owner_id:
                project.send_in_app_notification(
                    project.project_owner_id,
                    _("A new project '%s' has been initialized and you have been assigned as the Project Owner.") % project.name,
                    title=_("New Project Created")
                )
        return projects

    def action_initialize_warehouse(self):
        """ Used to repair legacy projects or manually force warehouse creation. """
        for project in self:
            if not project.warehouse_id:
                safe_code = (project.name[:3] if project.name else "PRJ").upper()
                warehouse = self.env['stock.warehouse'].create({
                ...
                project.warehouse_id = warehouse.id
                project.message_post(body=_("Dedicated Site Warehouse initialized: %s") % warehouse.name)
        return True
    ...
    def write(self, vals):
        ...
        # NOTIFICATION: Inspector Assigned
            ...
            for project in self:
                if project.inspector_id:
                    ...
                    if project.project_owner_id:
                        project.send_in_app_notification(
                            project.project_owner_id,
                            _("Inspector '%s' has been assigned to project '%s'.") % (project.inspector_id.name, project.name),
                            title=_("Inspector Assigned")
                        )
                    
                    # 2. Formal Email for Inspector
                    template = self.env.ref('construction_erp.email_template_inspector_assignment', raise_if_not_found=False)
                    if template:
    ...
    def action_advance_phase(self):
        ...
        for project in self:
            ...
            if next_phase:
                ...
                # In-app Notifications for PM and Owner
                if project.user_id:
                    project.send_in_app_notification(
                        project.user_id,
                        _("Phase '%s' of project '%s' has been successfully completed and advanced.") % (project.current_phase_id.name, project.name),
                        title=_("Project Phase Completed")
                    )
                if project.project_owner_id:
                    project.send_in_app_notification(
                        project.project_owner_id,
